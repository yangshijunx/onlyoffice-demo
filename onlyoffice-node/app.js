...
const cfgSignatureAuthorizationHeader = configServer.get(
  "token.authorizationHeader"
);
const cfgSignatureAuthorizationHeaderPrefix = configServer.get(
  "token.authorizationHeaderPrefix"
);
...
app.get("/", (req, res) => {
  // define a handler for default page
  try {
    req.docManager = new docManager(req, res);

    res.render("index", {
      // render index template with the parameters specified
...
app.get("/download", (req, res) => {
  ...
  if (!!userAddress && cfgSignatureEnable && cfgSignatureUseForRequest) {
    const authorization = req.get(cfgSignatureAuthorizationHeader);
    if (
      authorization &&
      authorization.startsWith(cfgSignatureAuthorizationHeaderPrefix)
    ) {
      var token = authorization.substring(
        cfgSignatureAuthorizationHeaderPrefix.length
      );
    }
    ...
    try {
      ...
      console.log(
        `checkJwtHeader error: name = ${err.name} message = ${err.message} token = ${token}`
      );
      res.sendStatus(403);
      return;
    }
...
app.post("/upload", (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (file == undefined) {
      // if file parameter is undefined
    ...
    fileSystem.rename(file.path, `${uploadDir}/${file.name}`, (err) => {
      ...
      if (err) {
        ...
        res.write(
          `{ "filename": "${file.name}", "documentType": "${documentType}" }`
...
app.post("/convert", (req, res) => {
  // define a handler for converting files
  req.docManager = new docManager(req, res);
  ...
  const callback = async function (err, res) {
    ...
    try {
      ...
      if (result != 100) {
        // if the status isn't 100
        writeResult(fileName, result, null); // write the origin file to the result object
        return;
      }

      const correctName = req.docManager.getCorrectName(
        fileUtility.getFileName(fileName, true) + newFileType
      ); // get the file name with a new extension

      const { status, data } = await urllib.request(newFileUri, {
        method: "GET",
      });

      if (status != 200)
        throw new Error(`Conversion service returned status: ${status}`);
      ...
      const userAddress = req.docManager.curUserHostAddress();
      const historyPath = req.docManager.historyPath(
        fileName,
        userAddress,
        true
      );
      const correctHistoryPath = req.docManager.historyPath(
        correctName,
        userAddress,
        true
...
app.get("/files", (req, res) => {
  // define a handler for getting files information
  try {
    req.docManager = new docManager(req, res);
...
app.get("/files/file/:fileId", (req, res) => {
...
app.delete("/file", (req, res) => {
  // define a handler for removing file
  try {
    req.docManager = new docManager(req, res);
    let fileName = req.query.filename;
    if (fileName) {
      // if the file name is defined
      ...
      req.docManager.cleanFolderRecursive(
        req.docManager.storagePath(""),
        false
...
app.get("/csv", (req, res) => {
  // define a handler for downloading csv files
...
app.post("/reference", (req, res) => {
  // define a handler for renaming file
  req.docManager = new docManager(req, res);
  ...
  if (referenceData) {
    ...
    if (instanceId === req.docManager.getInstanceId()) {
      ...
      if (
        userAddress === req.docManager.curUserHostAddress() &&
        req.docManager.existsSync(
          req.docManager.storagePath(fileKey.fileName, userAddress)
        )
      ) {
  ...
  if (!fileName && !!req.body.path) {
    ...
    if (
      req.docManager.existsSync(req.docManager.storagePath(path, userAddress))
    ) {
      fileName = path;
    }
  }
  ...
  const data = {
    ...
    directUrl: req.body.directUrl
      ? req.docManager.getDownloadUrl(fileName)
      : null,
    referenceData: {
      fileKey: JSON.stringify({
        fileName,
        userAddress: req.docManager.curUserHostAddress(),
      }),
  ...
  if (cfgSignatureEnable) {
    data.token = jwt.sign(data, cfgSignatureSecret, {
...
app.post("/track", async (req, res) => {
  // define a handler for tracking file changes
  req.docManager = new docManager(req, res);
  ...
  const processTrack = async function (response, body, fileName, userAddress) {
    ...
    const callbackProcessSave = async function (
      ...
      try {
        ...
        const storagePath = req.docManager.storagePath(
          newFileName,
          userAddress
        );
        ...
        if (historyPath == "") {
          ...
          historyPath = req.docManager.historyPath(
            newFileName,
            userAddress,
            true
          ); // create it
        ...
        const versionPath = req.docManager.versionPath(
          newFileName,
          userAddress,
          version
        ...
        if (downloadZip) {
          const path_changes = req.docManager.diffPath(
            newFileName,
            userAddress,
            version
        ...
        if (changeshistory) {
          const path_changes_json = req.docManager.changesPath(
            newFileName,
            userAddress,
            version
        ...
        const path_key = req.docManager.keyPath(
          newFileName,
          userAddress,
          version
        ); // get the path to the key.txt file
        ...
        fileSystem.renameSync(
          req.docManager.storagePath(fileName, userAddress),
          path_prev
        ); // and write it to the current path

        fileSystem.writeFileSync(storagePath, data);

        const forcesavePath = req.docManager.forcesavePath(
          newFileName,
          userAddress,
          false
    ...
    const processSave = async function (
      ...
      if (downloadExt != curExt) {
        ...
        newFileName = req.docManager.getCorrectName(
    ...
    const callbackProcessForceSave = async function (
      ...
      try {
        ...
        if (isSubmitForm) {
          // new file
          if (newFileName) {
            fileName = req.docManager.getCorrectName(
          ...
          forcesavePath = req.docManager.forcesavePath(
            fileName,
            userAddress,
            false
          );
      ...
      if (downloadExt != curExt) {
        ...
        try {
          documentService.getConvertedUriSync(
            ...
            async (err, data) => {
              if (err) {
                await callbackProcessForceSave(
                  downloadUri,
                  body,
                  fileName,
                  userAddress,
      ...
      if (checkJwtHeaderRes) {
        ...
        if (checkJwtHeaderRes.query) {
          ...
          if (checkJwtHeaderRes.query.filename) {
            fileName = fileUtility.getFileName(
              checkJwtHeaderRes.query.filename
            );
          }
        }
      }
  ...
  if (req.body.hasOwnProperty("status")) {
    // if the request body has status parameter
...
app.get("/editor-config", (req, res) => {
  ...
  try {
    ...
    if (type == "") {
      type = new RegExp(configServer.get("mobileRegEx"), "i").test(
        req.get("User-Agent")
      )
        ? "mobile"
        : "desktop";
    ...
    const templatesImageUrl = req.docManager.getTemplateImageUrl(
    ...
    if (fileExt != null) {
      var fileName = req.docManager.createDemo(
        !!req.query.sample,
        fileExt,
        userid,
        name,
    ...
    if (historyPath != "") {
      ...
      for (var i = 1; i <= countVersion; i++) {
        ...
        history.push(
          req.docManager.getHistory(fileName, changes, keyVersion, i)
          ...
          url:
            i == countVersion
              ...
              : `${req.docManager.getServerUrl(
                  true
                )}/history?fileName=${encodeURIComponent(
                  fileName
        ...
        if (
          i > 1 &&
          req.docManager.existsSync(
            req.docManager.diffPath(fileName, userAddress, i - 1)
          )
        ) {
          ...
          const changesUrl = `${req.docManager.getServerUrl(
            ...
            fileName
          )}&file=diff.zip&ver=${i - 1}&useraddress=${userAddress}`;
        ...
        if (i < countVersion) {
          const changesFile = req.docManager.changesPath(
            fileName,
            userAddress,
            i
          ); // get the path to the file with document changes
      ...
      history.push(
        req.docManager.getHistory(fileName, changes, keyVersion, countVersion)
    ...
    const argss = {
      ...
      editor: {
        ...
        chat: userid != "uid-0",
        coEditing:
          mode == "view" && userid == "uid-0"
            ? { mode: "strict", change: false }
            : null,
        comment:
          mode != "view" &&
          mode != "fillForms" &&
      ...
      dataInsertImage: {
        ...
        directUrl: !userDirectUrl
          ? null
          : `${req.docManager.getServerUrl()}/images/logo.png`,
      },
      dataCompareFile: {
        fileType: "docx",
      ...
      dataMailMergeRecipients: {
        ...
        directUrl: !userDirectUrl
          ? null
          : `${req.docManager.getServerUrl()}/csv`,
      },
      usersForMentions:
        user.id != "uid-0" ? users.getUsersForMentions(user.id) : null,
      usersForProtect:
        user.id != "uid-0" ? users.getUsersForProtect(user.id) : null,
    };
    ...
    if (cfgSignatureEnable) {
      app.render("config", argss, (err, html) => {
        ...
        if (err) {
          ...
          console.log("进没进这里222", argss.editor.token);
          argss.dataInsertImage.token = jwt.sign(
            argss.dataInsertImage,
            cfgSignatureSecret,
            { expiresIn: cfgSignatureSecretExpiresIn }
